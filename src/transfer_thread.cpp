...
void TransferThread::Enqueue(std::unique_ptr<RestJob> job)
{
	{
...
void TransferThread::Consume(int jobId, long long bytes)
{
	{
		std::lock_guard<std::mutex> const lock(m_mutex);
		m_consumeRequests.emplace_back(jobId, bytes);
	}
...
void TransferThread::Run()
{
	while (m_running)
	{
		...
		TakePending(incoming, cancelRequests, consumeRequests);
		AdmitIncoming(incoming);
		ApplyCancellations(cancelRequests);
		ApplyConsumed(consumeRequests);
		StartDueRetries();
...
void TransferThread::TakePending(
	std::vector<std::unique_ptr<RestJob>> &incoming,
	std::vector<int> &cancelRequests,
	std::vector<ConsumeRequest> &consumeRequests
)
{
	...
	consumeRequests.swap(m_consumeRequests);
}

void TransferThread::AdmitIncoming(std::vector<std::unique_ptr<RestJob>> &incoming)
{
...
RestJob *TransferThread::FindActive(int jobId)
{
	for (auto &job : m_active)
	{
...
void TransferThread::ApplyConsumed(const std::vector<ConsumeRequest> &consumeRequests)
{
	for (const ConsumeRequest &request : consumeRequests)
	{
		RestJob *job = FindActive(request.first);

		if (job != nullptr)
		{
			job->OnStreamConsumed(request.second);
		}
	}
}
...
void TransferThread::RemoveFinished()
{
	m_active.erase(
		std::remove_if(
			m_active.begin(), m_active.end(), [](const std::unique_ptr<RestJob> &job) { return job->IsFinished(); }
		),
		m_active.end()
	);
}
